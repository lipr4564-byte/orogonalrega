...
pending_requests: dict[str, dict] = {}     # для запросов владельцу
confirm_data: dict[str, str] = {}          # для подтверждения регистрации
slot_select_data: dict[str, str] = {}      # для кнопок выбора слота
unregister_data: dict[str, str] = {}       # для кнопок снятия
...
async def update_reg_message(bot, year: int):
    ...
    Если содержимое не изменилось – ничего не делает.
    Если сообщение удалено – сбрасывает ID и создаёт новое.
    """
    ...
    # Только если нет валидного msg_id
